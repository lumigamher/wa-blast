...
export default async function Home() {
  ...
  const startOfMonthTs = Math.floor(new Date(new Date().getFullYear(), new Date().getMonth(), 1).getTime() / 1000);

  const [recent, running, monthStats, templates] = await Promise.all([
    db.select().from(campaigns).where(eq(campaigns.orgId, orgId)).orderBy(desc(campaigns.createdAt)).limit(5),
    db
      .select()
      .from(campaigns)
      .where(sql`${campaigns.orgId} = ${orgId} AND ${campaigns.status} IN ('queued','sending')`)
    ...
    db
      ...
      .from(campaigns)
      .where(sql`${campaigns.orgId} = ${orgId} AND ${campaigns.createdAt} >= ${startOfMonthTs}`),
  ...
  const successRate = monthProcessed > 0 ? Math.round((Number(stats.sent) / monthProcessed) * 100) : 0;
    ...
    <div className="space-y-8">
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div className="space-y-1.5">
          ...
          <p className="text-sm text-muted-foreground">Tu panel de envíos masivos de WhatsApp.</p>
        </div>
        <div className="flex gap-2">
          <Link href="/plantillas/nueva" className={buttonVariants({ variant: "outline", size: "lg" })}>
            <PlusIcon className="size-4" />
            Nueva plantilla
          </Link>
          <Link href="/campanas/nueva" className={buttonVariants({ size: "lg" })}>
          ...
          <CardContent className="flex flex-wrap items-center justify-between gap-4 py-4">
            <div className="space-y-1">
              ...
              <div className="min-w-[260px]">
                <Progress
                  value={
                    runningCamp.total > 0 ? ((runningCamp.sent + runningCamp.failed) / runningCamp.total) * 100 : 0
                  }
                />
                <div className="mt-1 text-xs text-muted-foreground">
                  {runningCamp.sent + runningCamp.failed} / {runningCamp.total} procesados · {runningCamp.failed}{" "}
                  fallidos
                </div>
              </div>
            </div>
            <Link href={`/campanas/${runningCamp.id}`} className={buttonVariants({ size: "sm" })}>
              Ver detalle →
            </Link>
          </CardContent>
        </Card>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <MetricCard
          ...
          icon={XCircleIcon}
          color={Number(stats.failed) > 0 ? "text-amber-600" : "text-muted-foreground"}
        />
        <MetricCard
          ...
          hint={pending > 0 ? `${pending} en aprobación` : creds ? undefined : "Configura Meta"}
        />
      </div>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between gap-4">
          ...
          {recent.length > 0 && (
            <Link href="/campanas" className="text-sm text-primary hover:underline">
              Ver todas →
            </Link>
          )}
        </CardHeader>
                ...
                return (
                  <li key={c.id} className="rounded-md border bg-background p-3">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <Badge variant={c.status === "done" ? "default" : "secondary"}>{c.status}</Badge>
                    </div>
  ...
  return (
    <Card className="gap-2 py-5">
      <CardHeader className="flex flex-row items-center justify-between gap-2 pb-0">
        <CardDescription className="text-[11px] uppercase tracking-wider">{label}</CardDescription>
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-3 rounded-md border border-dashed py-10 text-center">
      ...
      <div className="space-y-1">
        ...
        <div className="text-xs text-muted-foreground">Crea tu primera campaña desde Nuevo envío.</div>
      </div>
      <Link href="/campanas/nueva" className={buttonVariants({ size: "sm", variant: "outline" })}>
        Empezar
      </Link>
    </div>
  );
