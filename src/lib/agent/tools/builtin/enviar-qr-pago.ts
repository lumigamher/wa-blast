...
export const enviarQrPago: AgentTool = {
  ...
  jsonSchema: {
    type: "object",
    properties: {
      type: { type: "string", description: "Tipo de medio (nequi, daviplata, bre_b, transferencia, link)" },
      methodId: { type: "string", description: "ID específico del método de pago (opcional si hay type)" },
    },
  },
  escalates: false,
  async run(args, ctx) {
    ...
    let selectedMethod;
    if (methodId) {
      selectedMethod = methods.find((m) => m.id === methodId && m.qrMediaAssetId);
    } else if (type) {
      selectedMethod = methods.find((m) => m.type.toLowerCase() === type.toLowerCase() && m.qrMediaAssetId);
    } else {
      selectedMethod = methods.find((m) => m.qrMediaAssetId);
    }

    if (!selectedMethod || !selectedMethod.qrMediaAssetId) {
    ...
    const [convRow] = await ctx.db.select().from(conversations).where(eq(conversations.id, ctx.conversationId));
    if (!convRow) {
    ...
    const uploadResult = await uploadMedia(settings, { bytes, mime: asset.mime });
