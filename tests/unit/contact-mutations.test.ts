...
describe("createContact", () => {
	it("normaliza el teléfono y crea el contacto", async () => {
		const { db } = makeTestDb();
		await seed(db);
		const res = await createContact(db, "o1", "CO", {
			phone: "300 123 4567",
			name: "Juan",
		});
...
describe("deleteContact", () => {
	it("borra el contacto y sus tags (cascade)", async () => {
		...
		await db
			.insert(tags)
			.values({ id: "t1", orgId: "o1", name: "VIP", color: "#fff" });
...
describe("setContactTagsForOrg", () => {
	it("reemplaza el set y descarta tags de otro org", async () => {
		...
		await db
			.insert(tags)
			.values({ id: "tX", orgId: "o2", name: "Ajeno", color: "#000" });
